...
**契约的后续状态（2026-08-29，P2.4 批次一 Step 4 核对）**：契约在阶段 1 当天已达成，此处只记录之后发生的位移，免得后人误以为契约被破坏。

- **契约列的 6 个源文件里只有 4 个真的 import 过门面**（`2edef68^` 实测）：`codex-pet-worker.ts`、`codex-pet-packaging.ts`、`codex-pet-generated-board-recovery.ts`、`codex-pet-recovery-finalizer.ts`。`codex-pet-failed-continuation.ts` 与 `server.ts` 从未直接 import，列进契约属保守的过度覆盖 —— 这正是上面执行结果写「4 个外部源文件 + 6 个外部测试文件」而本段写「6 个源文件 + 5 个测试文件」的原因，不是漏改。
- **6 个契约测试文件今天仍全部有效**：4 个自阶段 1 起逐字节未变（`codex-pet-board-version` / `codex-pet-failed-continuation` / `codex-pet-generated-board-recovery` / `codex-pet-recovery-finalizer`）；另 2 个被改过但改动人不是拆分任务 —— `codex-pet-runner-contract.test.ts` 由 P2.1 目录化改 import 路径（`1e32e62`、`39e42a1`），`codex-pet-runner.integration.test.ts` 另有 `7c98eb2`（2026-08-26「四份 look 修复循环合一为 `repairLook{A,B}Row`」）。**`7c98eb2` 是下方阶段 2 第一项的内容，已落地但阶段 2 仍未勾选，开阶段 2 前请先核对实际剩余范围。**
- **P2.4 批次一拆了契约里的 2 个文件**：`codex-pet-worker.ts` 1158 → 467 + 4 个兄弟文件（`ecde051`）、`codex-pet-packaging.ts` 863 → 27 行门面 + 3 个兄弟文件（`2abcd62`）。"一行都不改"从今天起不再描述这两个文件，但契约的**目的**未被破坏：对门面的依赖只是换了文件 —— `codex-pet-packaging-artifacts.ts` / `-run.ts` / `-shared.ts` 直接 import 门面，worker 侧四个文件经 P2.1 建立的 `workflow/codex-pet/index.js` 域门面间接 import。
- **11 个值 + 6 个类型今天全部仍在门面导出面上**（`codex-pet-runner.ts:163-197` 实测），且已是超集：后续功能又加了 12 个名字（`PoseBoardSalvage` / `codexPetJumpingTargetHeight` / `loadPoseBoardSalvage` / `poseBoardSalvageMetadata` / `poseBoardSalvageSlots` / `poseBoardSlotHealth` / `preferPoseBoardSalvage` / `atlasValidationErrorsWithoutCellScope` / `codexPetCoupledStandardRepairRows` / `repairRowsFromAtlasValidation` / `repairRowsFromDirectionContinuity` / `codexPetShouldMirrorRunningLeft`）。唯一换了老家的是 `CODEX_PET_BOARD_PROMPT_VERSION`，现由 `codex-pet-board-version.js` 转出，门面出口不变。
- **三条硬约束逐条复验通过**：① 四个信号异常类仍只在 `codex-pet-runner/runner-types.ts:68/215/222/232` 各有一个 `class` 定义点；② `emit` 的隐藏写路径完整活着（`runner-lease.ts:15-47`：`$transaction` 内先 `SELECT … FOR UPDATE` 行锁，再按 `workerId + status ∈ ACTIVE + cancelRequested=false` 做 lease CAS，失配抛 `CodexPetLeaseLostError`，随后 run 与 project 双双置 `repairing`），没被简化成纯事件函数；③ `MIRROR_NOT_SAFE` 字符串契约两端一致，抛在 `runner-standard-rows.ts:126`、匹配在 `codex-pet-runner.ts:425`。
