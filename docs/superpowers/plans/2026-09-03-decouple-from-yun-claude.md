...
> **2026-09-18 范围纠正（优先于下文历史方案）**：用户要求保留的是**整个生图模块**，包含通用生图、电商主图/详情长图、商品提取、形象照、万物试穿（含模特试穿）。下文把 ecom / portrait / try-on / ProductExtraction 及其接口、素材来源、数据表列为删除项，是错误的范围判断，不得继续执行。上述功能现已恢复；计费等其他已移除模块不随之恢复。下文数量及“全部放行”状态仅记录当时方案，不再代表这些生图场景的有效删除授权。
