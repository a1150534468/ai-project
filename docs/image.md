...
生图模块必须完整保留五个场景：**通用生图、电商图（主图与详情长图）、商品提取、形象照、万物试穿（含模特试穿）**。此前解耦误删的四个场景已恢复，包括菜单、工作台、接口、历史和素材库来源。下文主体介绍通用生图底层能力，不应据此缩减产品范围。

产品内部计费仍保持下线。恢复场景用本地零值兼容实现保留历史任务字段，不连接原计费服务；这不代表外部模型调用免费。历史章节中的扣费/退款描述仅用于理解旧实现。新增迁移仅重建八张被误删的表；已被旧删除迁移清掉的数据，需要备份另行恢复。
...
「双轨但单一服务」是刻意的：电商、桌宠等模块只需要依赖这一个 `image-service`，不各自造生图客户端，避免多套实现漂移（这正是电商模块整改时反复强调的；电商模块于 2026-09-18 纠正误删后恢复）。
