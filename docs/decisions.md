...
- **日期**：2026-09-04 立项，2026-09-05 更新验收判据（Phase 1~7+9 与 Phase 8 批次 A 已落地）
...
- **落选理由**：
  ...
  - **行级切除全历史** —— 产出的每棵树都是从未真实存在过的状态，编译不过、blame 指向的行
    在那个 commit 里并非那样。
- **后果**：
  - **终点不是 0，是「lockfile + 地板」**：`pnpm-lock.yaml` 的 6,687 行已实测只能靠真改依赖
    版本才会变（删掉从零重生成，产物与 `pnpm install --lockfile-only` 逐字节一致），
    那是升级不是解耦；再加上验收判据里那六类地板行。
  ...
  - **`prisma/migrations` 里 81 个历史迁移一个字不动** —— 它们是线上库的真实演化记录，改了
    `migrate deploy` 会对不上。代价是迁移目录里约 2,571 行上游代码留着，这是刻意付的。
...
- 上游行 **34,582**（2026-09-05 实测，含 `pnpm-lock.yaml` 6,687）。Phase 8 批次 A（分发面）
  已完成，剩余在 `presets.md`（批次 B）、api 保留模块（批次 C）与 `packages/db`（批次 D），
  清单见 ADR-012 引用的方案文档。
