...
- **日期**：2026-09-04（执行中，Phase 1~3 已落地）
...
- 剩约 4.5 万行代码逐字节来自上游导入，重写清单见 ADR-012 引用的方案文档 Phase 8。

---
## 已知架构债

- `apps/api/src/workflow/` 约 180 个文件、四条业务线（article / codex-pet / image / novel）。
- 路由注册平铺在 `server.ts`（20 个 register），可按域聚合为 Fastify 插件分组。
- 两套队列（租约 + BullMQ）并存，长期宜统一到租约模式。
- 剩约 4.5 万行代码逐字节来自上游导入，重写清单见 ADR-012。
