...
- 上游行 **34,471**（2026-09-05 实测，含 `pnpm-lock.yaml` 6,669）。Phase 8 批次 A（分发面）
  与随后的 A8 清扫批已完成，剩余在 `presets.md`（批次 B）、api 保留模块（批次 C）与
  `packages/db`（批次 D），清单见 ADR-012 引用的方案文档。
