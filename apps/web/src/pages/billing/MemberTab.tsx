  ...
  return (
    <div>
      ...
      <div id="member-plans" className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-ink-tertiary">一次性购买 · 到期不自动续费</span>
      </div>
      {membershipCards.length > 0 ? (
          ...
          {membershipCards.map((card) => {
            ...
            return (
              <StaggerItem key={card.id}>
                <div className={`bg-surface rounded-2xl p-5 border relative ${isCurrent ? "border-brand ring-2 ring-brand" : "border-hairline-subtle"}`}>
                  ...
                  <div className="mt-2 flex items-baseline gap-1">
                    ...
                    <span className="text-xs text-ink-tertiary">/ 有效期 {card.durationDays} 天</span>
                  </div>
                  <div className="mt-4 rounded-xl bg-brand-soft p-3">
                    ...
                    <p className="text-[11px] text-ink-secondary mt-1">{label}刷新 · 到期未用清零</p>
                  </div>
                  <p className="text-[12px] text-ink-tertiary mt-3">整个周期共发放 <b className="text-ink">{total.approx ? "约 " : ""}{total.points.toLocaleString()}</b> 点</p>
                  {quota && <p className="text-[12px] text-ink-tertiary mt-1">{quota}</p>}
                  <button
                    onClick={() => onBuyMembership(card)}
                    disabled={loading}
                    className={`w-full mt-4 py-2.5 text-[13px] font-medium rounded-full transition-all disabled:opacity-50 ${isCurrent ? "bg-surface-subtle text-ink-secondary " : "bg-brand text-white "}`}
                  >
        ...
        <div className="rounded-2xl border border-dashed border-hairline-subtle p-6 text-center text-sm text-ink-tertiary">暂无可购买会员</div>
      )}
    </div>
  );
}
