    ...
    <div>
      ...
      <div className="bg-surface rounded-2xl p-4 mb-5 border border-hairline-subtle flex flex-wrap items-center gap-x-8 gap-y-3">
        <div>
          ...
          {temp !== null && <span className="ml-1 text-[11px] text-ink-tertiary">（含临时 {temp.toLocaleString()}）</span>}
        </div>
        <div>
          <span className="text-[11px] text-ink-tertiary font-medium">视频点余额</span>
          <span className="ml-2 text-lg font-extrabold text-ink">{video === null ? "同步中" : video.toLocaleString()}</span>
        </div>
        <div className="ml-auto inline-flex rounded-full bg-surface-subtle p-1">
          {(["alipay", "wxpay"] as PaymentMethod[]).map((method) => (
            <button
              ...
              disabled={p.loading}
              className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition-all ${p.paymentMethod === method ? "bg-surface text-brand-ink shadow-sm" : "text-ink-secondary "}`}
            >
      ...
      <div className="mb-6">
        ...
        <Stagger className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
          ...
          {p.packages.length === 0 && (
            <div className="sm:col-span-2 xl:col-span-4 bg-surface rounded-2xl p-6 border border-dashed border-hairline-subtle text-sm text-ink-secondary">
        ...
        <div className="bg-surface rounded-2xl p-6 border border-hairline-subtle">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-tertiary text-sm">¥</span>
      ...
      <div className="bg-surface rounded-2xl p-6 border border-hairline-subtle">
        ...
        {p.usageRows.length > 0 ? (
          <Stagger className="space-y-2">
            {p.usageRows.map((row) => (
              <StaggerItem key={row.operationId}>
                <div className="flex items-center justify-between gap-3 rounded-xl border border-hairline-subtle px-4 py-3">
                  ...
                  <div className="text-right flex-none">
                    ...
                    <p className="text-[10px] text-ink-tertiary mt-1">{row.status === "settled" ? "已结算" : row.status === "reserved" ? "结算中" : row.status}</p>
                  </div>
                </div>
              </StaggerItem>
            ))}
          ...
          <div className="rounded-xl border border-dashed border-hairline-subtle p-6 text-center text-sm text-ink-tertiary">暂无消耗记录</div>
        )}
      </div>
    </div>
  );
