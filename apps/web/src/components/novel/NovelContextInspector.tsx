...
export function NovelContextInspector({ token, projectId, chapter, workbench, isReviewSaving, onSaveReview, onAnalyze }: { readonly token: string; readonly projectId: string; readonly chapter: NovelChapter | null; readonly workbench: NovelWorkbenchPayload | null; readonly isReviewSaving: boolean; readonly onSaveReview: (payload: { status?: "pending" | "approved" | "revise"; reviewNotes?: string; regenerateAi?: boolean }) => void; readonly onAnalyze: () => void }) {
  ...
  return (
    <aside className="flex min-h-0 flex-col border-l border-[#dfe5e2] bg-[#fbfcfc]">
      <div className="border-b border-[#e5eae8] px-3 pt-3"><div className="flex gap-1 overflow-x-auto pb-2 [scrollbar-width:none]">{TABS.map((item) => <button key={item.id} type="button" onClick={() => setTab(item.id)} title={item.label} className={`flex h-8 min-w-8 items-center justify-center gap-1 rounded-lg px-2 text-[10px] font-semibold transition ${tab === item.id ? "bg-brand-soft text-brand-ink ring-1 ring-brand/20" : "text-[#74807c] hover:bg-[#eef2f0]"}`}><Icon icon={item.icon} /><span className="hidden 2xl:inline">{item.label}</span></button>)}</div></div>
