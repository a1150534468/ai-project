  ...
  return (
    <aside className="hidden min-h-0 flex-col border-l border-[#dfe5e2] bg-[#fbfcfc] xl:flex">
      ...
      <div className="min-h-0 flex-1 overscroll-contain overflow-y-auto p-3 [scrollbar-gutter:stable] [scrollbar-width:thin]">
        ...
        {tab === "world" && <div className="grid gap-2">{worldDimensions.map((item, index) => <article key={text(item.id) || index} className="rounded-xl border border-[#e2e7e5] bg-white p-3"><p className="text-xs font-semibold text-ink">{text(item.title)}</p><p className="mt-2 text-[10px] leading-4 text-[#707b76]">{text(item.summary) || JSON.stringify(item.details)}</p></article>)}{(setup?.locations ?? []).slice(0, 8).map((item, index) => <article key={text(item.id) || `location:${index}`} className="rounded-xl border border-[#e2e7e5] bg-white p-3"><p className="flex items-center gap-1 text-xs font-semibold text-ink"><Icon icon="mdi:map-marker-outline" className="text-brand-ink" />{text(item.name)}</p><p className="mt-1 text-[10px] leading-4 text-[#707b76]">{text(item.description)}</p></article>)}{!worldDimensions.length && !setup?.locations.length && <Empty>Bible 和地图生成后显示世界资料</Empty>}</div>}
