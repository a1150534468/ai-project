...
export function NovelChapterDesk({
  ...
  // 提交中连取消按钮都是禁的，Esc 跟着禁 —— 这时候关掉窗，改写照样在后台跑
  const rewriteDialog = useDialog({
    open: rewriteOpen && activeRewriteSelection !== null,
    onClose: isRewriting ? undefined : () => setRewriteOpen(false),
    label: "局部改写",
  });
