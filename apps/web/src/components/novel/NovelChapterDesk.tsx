...
export function NovelChapterDesk({
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col bg-white">
      <header className="flex-none border-b border-[#e3e8e6] px-4 py-3 sm:px-5">
        <div className="flex flex-wrap items-start gap-3">
          <div className="min-w-[220px] flex-[1_1_320px]"><div className="flex min-w-0 items-center gap-2"><span className="shrink-0 rounded-md bg-[#f0f3f2] px-2 py-1 text-[10px] font-semibold text-[#73807b]">第 {chapter?.chapterIndex ?? "-"} 章</span><input value={chapterTitle} onChange={(event) => onTitleChange(event.currentTarget.value)} disabled={!chapter} placeholder="章节标题" aria-label="章节标题" className="min-w-0 flex-1 rounded-none border-0 bg-transparent p-0 text-base font-semibold text-ink outline-none shadow-none focus:border-0 focus:shadow-none disabled:text-[#8d9693]" /></div><p className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-[#8b9491]"><Icon icon={saveStatus === "error" ? "mdi:alert-circle-outline" : "mdi:cloud-check-outline"} className={saveStatus === "saving" ? "animate-pulse text-brand-ink" : ""} />{statusText}<span>·</span><span>{words.toLocaleString("zh-CN")} 字</span>{changed && <><span>·</span><span className="text-brand-ink">已修改 AI 原稿</span></>}</p></div>
          <div className="ml-auto flex flex-[1_1_auto] flex-wrap items-center justify-end gap-2"><label className="flex h-9 items-center gap-1.5 rounded-lg border border-[#d9dfdd] bg-[#fafbfb] px-2.5 text-[10px] font-semibold text-[#77817d] transition focus-within:border-brand/60 focus-within:ring-2 focus-within:ring-brand/10">目标<input value={targetChars} onChange={(event) => onTargetCharsChange(event.currentTarget.value)} inputMode="numeric" aria-label="目标字数" className="w-12 rounded-none border-0 bg-transparent p-0 text-right text-xs font-semibold text-ink outline-none shadow-none focus:border-0 focus:shadow-none" />字</label><button type="button" onClick={() => setRewriteOpen(true)} disabled={!activeRewriteSelection || saveStatus === "saving" || isGenerating || isRewriting} title={activeRewriteSelection ? `改写已选中的 ${rewriteChars} 字` : "请先在正文中选择一段文字"} className="flex h-9 items-center gap-1.5 rounded-lg border border-[#d9dfdd] px-3 text-xs font-semibold text-[#58635f] disabled:opacity-40"><Icon icon={isRewriting ? "mdi:loading" : "mdi:text-box-edit-outline"} className={isRewriting ? "animate-spin" : ""} />{isRewriting ? "改写中" : "局部改写"}</button><button type="button" onClick={onAnalyze} disabled={!chapter?.content} className="flex h-9 items-center gap-1.5 rounded-lg border border-[#d9dfdd] px-3 text-xs font-semibold text-[#58635f] disabled:opacity-40"><Icon icon="mdi:shield-search-outline" />AI 审阅</button><button type="button" onClick={onGenerate} disabled={isGenerating || isRewriting} className="flex h-9 items-center gap-1.5 rounded-lg bg-brand px-4 text-xs font-semibold text-white shadow-sm disabled:opacity-45"><Icon icon={isGenerating ? "mdi:loading" : chapter?.content ? "mdi:refresh" : "mdi:creation-outline"} className={isGenerating ? "animate-spin" : ""} />{isGenerating ? "提交中" : chapter?.content ? "重写正文" : "生成正文"}</button></div>
        </div>
        ...
        ] as const).map(([id, label, icon]) => <button key={id} type="button" onClick={() => setTab(id)} className={`flex h-8 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold ${tab === id ? "bg-brand-soft text-brand-ink" : "text-[#737d79] "}`}><Icon icon={icon} />{label}</button>)}</nav>
      </header>

      <div className="min-h-0 flex-1 overscroll-contain overflow-y-auto [scrollbar-gutter:stable] [scrollbar-width:thin]">
        {!chapter ? <div className="grid min-h-[560px] place-items-center px-6 text-center"><div><Icon icon="mdi:book-open-page-variant-outline" className="mx-auto text-5xl text-[#a7aeab]" /><h3 className="mt-3 text-sm font-semibold text-[#56615d]">从左侧选择一个章节</h3><p className="mt-1 text-xs text-[#8c9592]">章节规划、正文与情报会在这里展开。</p></div></div> : tab === "prose" ? <div data-testid="novel-chapter-prose-layout" className="mx-auto flex min-h-full w-full max-w-[clamp(920px,82vw,1480px)] flex-col gap-4 px-[clamp(1.25rem,4vw,5rem)] py-[clamp(1.25rem,3vh,2.5rem)]">
          ...
          <textarea value={chapterContent} onChange={(event) => onContentChange(event.currentTarget.value)} onSelect={(event) => captureRewriteSelection(event.currentTarget)} placeholder="在这里写作，或点击右上角生成正文……" className="min-h-[320px] w-full flex-1 resize-none overflow-y-auto border-0 bg-transparent font-serif text-[16px] leading-[2.05] text-ink outline-none [scrollbar-gutter:stable] [scrollbar-width:thin] placeholder:text-[#b2b8b5]" />
          <div className="flex flex-none items-center justify-between border-t border-[#edf0ef] pt-3 text-[10px] text-[#909895]"><span>正文编辑器 · 自动保存</span><span>{words.toLocaleString("zh-CN")} 字 · 目标 {targetChars || "-"}</span></div>
        </div> : tab === "plan" ? <div className="mx-auto grid w-full max-w-[980px] gap-4 p-5 lg:grid-cols-2 lg:p-7"><label className="grid gap-2 rounded-2xl border border-[#e1e6e4] bg-[#fbfcfc] p-4 text-xs font-semibold text-[#4f5a56]">章节大纲<textarea value={chapterOutline} onChange={(event) => onOutlineChange(event.currentTarget.value)} rows={14} className="mt-1 resize-y rounded-xl border border-[#d9dfdd] bg-white p-3 text-sm font-normal leading-6 outline-none focus:border-brand/50" /></label><label className="grid gap-2 rounded-2xl border border-[#e1e6e4] bg-[#fbfcfc] p-4 text-xs font-semibold text-[#4f5a56]">生成约束<textarea value={generationHint} onChange={(event) => onGenerationHintChange(event.currentTarget.value)} rows={14} placeholder="必须出现 / 不得出现 / 视角 / 场景约束" className="mt-1 resize-y rounded-xl border border-[#d9dfdd] bg-white p-3 text-sm font-normal leading-6 outline-none focus:border-brand/50" /></label></div> : tab === "compare" ? <div className="grid min-h-[620px] gap-px bg-[#e1e6e4] lg:grid-cols-2"><section className="bg-[#fafbfb] p-5 lg:p-7"><div className="flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">AI 原稿</h3><span className="text-[10px] text-[#89928f]">{Array.from(original.replace(/\s+/gu, "")).length} 字</span></div><p className="mt-5 whitespace-pre-wrap font-serif text-sm leading-7 text-[#4e5854]">{originalPreview}</p></section><section className="bg-white p-5 lg:p-7"><div className="flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">当前版本</h3><span className="text-[10px] text-brand-ink">{changed ? "有人工修改" : "与原稿一致"}</span></div><p className="mt-5 whitespace-pre-wrap font-serif text-sm leading-7 text-ink">{chapterContent || "暂无正文"}</p></section></div> : tab === "prompts" ? <div className="grid min-h-[620px] lg:grid-cols-[300px_minmax(0,1fr)]"><aside className="border-r border-[#e1e6e4] bg-[#fafbfb] p-4"><div className="flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">实际发送记录</h3><span className="text-[10px] text-[#89928f]">{promptRequests.length} 次</span></div>{promptError && <p className="mt-3 rounded-lg bg-red-50 p-2 text-xs text-red-700">{promptError}</p>}<div className="mt-3 grid gap-2">{promptRequests.map((request) => <button key={request.id} type="button" onClick={() => setSelectedPromptId(request.id)} className={`rounded-xl border p-3 text-left ${selectedPrompt?.id === request.id ? "border-brand/40 bg-brand-soft" : "border-[#e1e6e4] bg-white"}`}><span className="block text-xs font-semibold">{request.targetKind === "chapterRewrite" ? "局部改写" : request.attempt > 1 ? `第 ${request.attempt} 次生成` : "正文生成"}</span><span className="mt-1 block text-[10px] text-[#89928f]">{new Date(request.createdAt).toLocaleString("zh-CN")} · {request.model}</span><span className={`mt-1 block text-[10px] ${request.status === "failed" ? "text-red-600" : "text-brand-ink"}`}>{request.status}</span></button>)}{!promptRequests.length && !promptError && <p className="py-10 text-center text-xs leading-5 text-[#89928f]">本章生成于提示词审计功能上线前，暂无可验证的原始请求。</p>}</div></aside><section className="min-w-0 p-5 lg:p-7">{selectedPrompt ? <div><div className="flex flex-wrap gap-2 text-[10px] text-[#69736f]"><span className="rounded-full bg-brand-soft px-2 py-1 text-brand-ink">实际发送</span><span className="rounded-full bg-[#f1f3f2] px-2 py-1">模型 {selectedPrompt.model}</span><span className="rounded-full bg-[#f1f3f2] px-2 py-1">Temperature {selectedPrompt.temperature ?? "默认"}</span><span className="rounded-full bg-[#f1f3f2] px-2 py-1">Max tokens {selectedPrompt.maxTokens}</span>{selectedPrompt.templateVersion && <span className="rounded-full bg-[#f1f3f2] px-2 py-1">模板 v{selectedPrompt.templateVersion}</span>}</div><div className="mt-5 grid gap-5"><section><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">System prompt</h3><button type="button" onClick={() => void navigator.clipboard?.writeText(selectedPrompt.systemPrompt)} className="text-[10px] font-semibold text-brand-ink">复制</button></div><pre className="max-h-52 overflow-auto whitespace-pre-wrap rounded-xl bg-[#111418] p-4 text-xs leading-6 text-[#d7e0e8]">{selectedPrompt.systemPrompt}</pre></section><section><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">User prompt</h3><button type="button" onClick={() => void navigator.clipboard?.writeText(selectedPrompt.userPrompt)} className="text-[10px] font-semibold text-brand-ink">复制</button></div><pre className="max-h-[520px] overflow-auto whitespace-pre-wrap rounded-xl border border-[#dfe5e2] bg-[#fbfcfc] p-4 text-xs leading-6 text-ink">{selectedPrompt.userPrompt}</pre></section></div></div> : <div className="grid min-h-[520px] place-items-center text-sm text-[#89928f]">请选择一条提示词记录</div>}</section></div> : <div className="grid min-h-[620px] lg:grid-cols-[260px_minmax(0,1fr)]"><aside className="border-r border-[#e1e6e4] bg-[#fafbfb] p-4"><div className="flex items-center justify-between"><h3 className="text-xs font-semibold text-[#4b5652]">版本历史</h3><span className="text-[10px] text-[#89928f]">{versions.length} 个版本</span></div>{versionError && <p className="mt-3 rounded-lg bg-red-50 p-2 text-xs text-red-700">{versionError}</p>}<div className="mt-3 grid gap-2">{versions.map((version, index) => <button key={version.id} type="button" onClick={() => setSelectedVersionId(version.id)} className={`rounded-xl border p-3 text-left ${selectedVersion?.id === version.id ? "border-brand/40 bg-brand-soft" : "border-[#e1e6e4] bg-white"}`}><span className="block text-xs font-semibold">版本 {versions.length - index}</span><span className="mt-1 block text-[10px] text-[#89928f]">{new Date(version.createdAt).toLocaleString("zh-CN")} · {version.billableChars} 字</span></button>)}{!versions.length && <p className="py-10 text-center text-xs text-[#89928f]">编辑或生成正文后会自动留档</p>}</div></aside><section className="p-5 lg:p-7"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-ink">历史版本预览</h3><p className="mt-1 text-xs text-[#89928f]">恢复前会自动保存当前正文</p></div><button type="button" onClick={() => void restoreVersion()} disabled={!selectedVersion || versionBusy} className="h-9 rounded-lg bg-brand px-3 text-xs font-semibold text-white disabled:opacity-40">{versionBusy ? "恢复中" : "恢复此版本"}</button></div><p className="mt-6 whitespace-pre-wrap font-serif text-sm leading-8 text-ink">{selectedVersion?.content || "请选择历史版本"}</p></section></div>}
      </div>
      {rewriteOpen && activeRewriteSelection && <div className="fixed inset-0 z-[90] grid place-items-center bg-[#13201c]/45 p-4" role="dialog" aria-modal="true" aria-label="局部改写"><div className="w-full max-w-xl overflow-hidden rounded-2xl border border-white/60 bg-white shadow-2xl"><header className="flex items-start justify-between border-b border-[#e5e9e7] px-5 py-4"><div><h3 className="flex items-center gap-2 text-sm font-semibold text-ink"><Icon icon="mdi:text-box-edit-outline" className="text-brand-ink" />局部改写</h3><p className="mt-1 text-xs text-[#818b87]">只替换当前选区，前后正文和章节版本都会保留。</p></div><button type="button" onClick={() => setRewriteOpen(false)} disabled={isRewriting} className="grid h-8 w-8 place-items-center rounded-lg text-[#727c78] disabled:opacity-40" aria-label="关闭"><Icon icon="mdi:close" /></button></header><div className="grid gap-4 p-5"><div className="rounded-xl border border-[#e1e6e4] bg-[#f7f9f8] p-3"><div className="flex items-center justify-between text-[10px] font-semibold text-[#7d8783]"><span>已选正文</span><span>{rewriteChars} 字</span></div><p className="mt-2 max-h-32 overflow-y-auto whitespace-pre-wrap font-serif text-xs leading-6 text-[#4b5652]">{activeRewriteSelection.text}</p></div><div><p className="mb-2 text-xs font-semibold text-[#4b5652]">改写方向</p><div className="mb-2 flex flex-wrap gap-2">{["润色表达，保持原意", "增强冲突与叙事张力", "精简重复，加快节奏", "扩写动作、感官与细节"].map((preset) => <button key={preset} type="button" onClick={() => setRewriteInstruction(preset)} className="rounded-full border border-[#dce2df] px-2.5 py-1 text-[10px] text-[#68736f] ">{preset}</button>)}</div><textarea value={rewriteInstruction} onChange={(event) => setRewriteInstruction(event.currentTarget.value)} maxLength={2000} rows={4} autoFocus className="w-full resize-y rounded-xl border border-[#d9dfdd] px-3 py-2 text-sm leading-6 text-ink outline-none focus:border-brand/50" placeholder="例如：保留事实不变，增强人物压迫感，减少解释性句子……" /></div></div><footer className="flex items-center justify-between border-t border-[#e8ecea] bg-[#fafbfb] px-5 py-3"><span className="text-[10px] text-[#89928f]">提交后由独立 Novel Worker 执行</span><div className="flex gap-2"><button type="button" onClick={() => setRewriteOpen(false)} disabled={isRewriting} className="h-9 rounded-lg border border-[#d9dfdd] px-3 text-xs font-semibold text-[#68736f] disabled:opacity-40">取消</button><button type="button" onClick={() => void submitRewrite()} disabled={isRewriting || !rewriteInstruction.trim()} className="flex h-9 items-center gap-1.5 rounded-lg bg-brand px-4 text-xs font-semibold text-white disabled:opacity-40"><Icon icon={isRewriting ? "mdi:loading" : "mdi:creation-outline"} className={isRewriting ? "animate-spin" : ""} />{isRewriting ? "提交中" : "开始改写"}</button></div></footer></div></div>}
    </section>
  );
}
