...
export function NovelWorkbenchShell({
  ...
  return (
    <section className="flex min-h-[calc(100dvh-64px)] flex-col overflow-hidden bg-[#f2f5f4] text-[#26302d]">
      ...
      <main className="min-h-0 flex-1">
        {workspace === "writing" ? <div className={`grid h-full min-h-[720px] ${leftOpen && rightOpen ? "xl:grid-cols-[270px_minmax(520px,1fr)_330px]" : leftOpen ? "xl:grid-cols-[270px_minmax(520px,1fr)]" : rightOpen ? "xl:grid-cols-[minmax(520px,1fr)_330px]" : "grid-cols-1"}`}>
        ...
        </div> : <div className="h-full overflow-y-auto [scrollbar-width:thin]">{workspace === "story" ? <div className="p-4 sm:p-6"><NovelIntelligenceWorkspace token={token} projectId={detail.project.id} showPrompts={false} /></div> : workspace === "bible" ? <NovelBibleWorkspace token={token} projectId={detail.project.id} onOpenSetup={onOpenSetup} /> : workspace === "autopilot" ? <div className="p-4 sm:p-6"><NovelRunCockpit token={token} projectId={detail.project.id} nextChapter={(chapters.at(-1)?.chapterIndex ?? 0) + 1} onProjectChanged={onRefresh} /></div> : <div className="p-4 sm:p-6"><NovelPromptWorkbench token={token} projectId={detail.project.id} /></div>}</div>}
      </main>
    </section>
  );
}
