    ...
    <section className="rounded-[14px] border border-hairline-subtle bg-white p-5">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[240px_minmax(0,1fr)_240px]">
        ...
        <div className="grid gap-3">
          <div className="grid gap-3 sm:grid-cols-3"><label className="grid gap-1 text-xs font-semibold text-ink-secondary">节点 Key<input value={nodeKey} onChange={(event) => setNodeKey(event.target.value)} className="h-9 rounded-lg border border-hairline px-3 text-sm" /></label><label className="grid gap-1 text-xs font-semibold text-ink-secondary">名称<input value={name} onChange={(event) => setName(event.target.value)} className="h-9 rounded-lg border border-hairline px-3 text-sm" /></label><label className="grid gap-1 text-xs font-semibold text-ink-secondary">分类<input value={category} onChange={(event) => setCategory(event.target.value)} className="h-9 rounded-lg border border-hairline px-3 text-sm" /></label></div>
          <textarea value={content} onChange={(event) => setContent(event.target.value)} rows={18} className="w-full resize-y rounded-lg border border-hairline bg-brand/5 p-4 font-mono text-sm leading-6 outline-none focus:border-brand/60" />
          <div className="flex flex-wrap items-end gap-3"><label className="grid gap-1 text-xs font-semibold text-ink-secondary">平台模型<select value={model} onChange={(event) => setModel(event.target.value)} className="h-9 min-w-48 rounded-lg border border-hairline px-3 text-sm"><option value="">系统默认</option>{models.map((item) => <option key={item.model} value={item.model}>{item.displayName || item.model}</option>)}</select></label><label className="grid gap-1 text-xs font-semibold text-ink-secondary">Temperature<input value={temperature} onChange={(event) => setTemperature(event.target.value)} className="h-9 w-24 rounded-lg border border-hairline px-3 text-sm" /></label><button type="button" onClick={() => void save()} disabled={busy} className="h-9 rounded-lg bg-brand px-4 text-sm font-semibold text-white disabled:opacity-50">{busy ? "保存中" : "保存新版本"}</button>{message && <span className="text-xs text-ink-secondary">{message}</span>}</div>
        </div>
        <div><h4 className="text-sm font-semibold text-ink">版本历史</h4><div className="mt-3 grid max-h-[430px] gap-2 overflow-y-auto [scrollbar-width:thin]">{selected?.versions.map((version) => <div key={version.id} className="rounded-lg border border-hairline-subtle p-3"><div className="flex items-center justify-between"><span className="text-sm font-semibold">v{version.version}</span>{version.version !== selected.activeVersion && <button type="button" onClick={() => void rollback(version.version)} disabled={busy} className="text-xs font-semibold text-brand-ink">回滚</button>}</div><p className="mt-1 text-xs text-ink-tertiary">{version.changeNote || new Date(version.createdAt).toLocaleString("zh-CN")}</p></div>)}{!selected?.versions.length && <p className="text-xs text-ink-tertiary">保存后生成版本记录</p>}</div></div>
      </div>
    </section>
  );
}
