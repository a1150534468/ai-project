...
function chapterState(chapter: NovelChapter | undefined): { label: string; className: string } {
  if (!chapter?.content) return { label: "待写", className: "bg-[#edf0ef] text-ink-tertiary" };
  ...
  return (
    <aside className="hidden min-h-0 flex-col border-r border-[#dfe5e2] bg-[#fbfcfc] xl:flex">
      <div className="border-b border-[#e5eae8] p-3">
        ...
        <label className="mt-3 flex h-9 items-center gap-2 rounded-lg border border-[#d9dfdd] bg-white px-2.5 text-xs text-ink-tertiary transition focus-within:border-brand/60 focus-within:ring-2 focus-within:ring-brand/10"><Icon icon="mdi:magnify" /><input value={query} onChange={(event) => setQuery(event.currentTarget.value)} placeholder="搜索章节或节点" className="min-w-0 flex-1 rounded-none border-0 bg-transparent p-0 outline-none shadow-none focus:border-0 focus:shadow-none" /></label>
        <div className="mt-2 grid grid-cols-2 rounded-lg bg-[#edf1ef] p-1"><button type="button" onClick={() => setFlat(false)} className={`h-7 rounded-md text-[10px] font-semibold ${!flat ? "bg-white text-brand-ink shadow-sm" : "text-ink-tertiary"}`}>结构树</button><button type="button" onClick={() => setFlat(true)} className={`h-7 rounded-md text-[10px] font-semibold ${flat ? "bg-white text-brand-ink shadow-sm" : "text-ink-tertiary"}`}>平铺章节</button></div>
      </div>
