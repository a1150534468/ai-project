...
export function NovelRunCockpit({
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col gap-4 overflow-hidden">
      <div className="flex-none rounded-[14px] border border-hairline-subtle bg-white p-4 shadow-[0_16px_44px_rgba(15,23,42,0.055)] sm:p-5">
      ...
      ].map(([label, value, hint, icon]) => <article key={String(label)} className="rounded-2xl border border-hairline-subtle bg-white p-5"><Icon icon={String(icon)} className="text-2xl text-brand-ink" /><p className="mt-3 text-xs font-semibold text-ink-tertiary">{label}</p><p className="mt-1 text-3xl font-semibold">{value}</p><p className="mt-2 text-xs text-ink-secondary">{hint}</p></article>)}</div>}

      {view === "dashboard" && <section className="min-h-0 flex-1 overflow-y-auto overscroll-contain rounded-2xl border border-hairline-subtle bg-white p-5 [scrollbar-gutter:stable] [scrollbar-width:thin]"><div><h3 className="font-semibold">章节张力 / 质量仪表盘</h3><p className="mt-1 text-xs text-ink-tertiary">柱高为总张力，圆点显示质量门禁分。</p></div><div className="mt-6"><NovelScoreTrend rows={dashboard?.tensionCurve ?? []} /></div></section>}
      ...
      {view === "cockpit" && <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain xl:overflow-hidden">
        ...
        <section className="flex min-h-[420px] min-w-0 flex-col overflow-hidden rounded-[14px] border border-hairline-subtle bg-white p-4 xl:min-h-0">
