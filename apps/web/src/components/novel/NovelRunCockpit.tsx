...
export function NovelRunCockpit({
  ...
  return (
    <section className="flex h-full min-h-0 min-w-0 flex-col gap-4 overflow-hidden">
      <div className="flex-none rounded-[14px] border border-hairline-subtle bg-surface p-4 shadow-[0_16px_44px_rgba(15,23,42,0.055)] sm:p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap items-end gap-2">
            <label className="grid gap-1 text-xs font-semibold text-ink-secondary">目标章节<input value={targetChapters} onChange={(event) => setTargetChapters(event.target.value)} className="h-9 w-24 rounded-lg border border-hairline px-3 text-sm text-ink" /></label>
            <label className="grid gap-1 text-xs font-semibold text-ink-secondary">每章字数<input value={targetChars} onChange={(event) => setTargetChars(event.target.value)} className="h-9 w-24 rounded-lg border border-hairline px-3 text-sm text-ink" /></label>
            <label className="flex h-9 items-center gap-2 rounded-lg border border-hairline px-3 text-xs font-semibold"><input type="checkbox" checked={autoReview} onChange={(event) => setAutoReview(event.target.checked)} />质量通过后自动继续</label>
            ...
            {activeRun && ACTIVE_STATUSES.has(activeRun.status) && <button type="button" onClick={() => void control("pause")} disabled={Boolean(busy)} className="h-9 rounded-lg border border-warning/30 px-4 text-sm font-semibold text-warning-ink">暂停</button>}
            ...
            {activeRun && !["completed", "cancelled"].includes(activeRun.status) && <button type="button" onClick={() => void control("cancel")} disabled={Boolean(busy)} className="h-9 rounded-lg border border-danger/30 px-4 text-sm font-semibold text-danger-ink">停止</button>}
          </div>
        </div>
        <nav className="mt-5 flex gap-1 overflow-x-auto rounded-xl bg-surface-muted p-1 [scrollbar-width:none]">{([
          ["cockpit", "全托管驾驶", "mdi:steering"], ["governance", "总编辑治理", "mdi:shield-crown-outline"], ["dashboard", "数据仪表盘", "mdi:chart-box-outline"],
        ] as const).map(([id, label, icon]) => <button key={id} type="button" onClick={() => setView(id)} className={`flex h-9 shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold ${view === id ? "bg-surface text-brand-ink shadow-sm" : "text-ink-secondary"}`}><Icon icon={icon} />{label}</button>)}</nav>
        {error && <p className="mt-4 rounded-lg bg-danger/10 px-3 py-2 text-sm text-danger-ink">{error}</p>}
        {view === "cockpit" && <><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            <div key={label} className="rounded-xl border border-hairline-subtle bg-surface-subtle p-3">
        ...
        <div className="mt-4 h-2 overflow-hidden rounded-full bg-surface-muted"><div className="h-full rounded-full bg-brand transition-all" style={{ width: `${percent}%` }} /></div></>}
      </div>
      ...
      ].map(([label, value, hint, icon]) => <article key={String(label)} className="rounded-2xl border border-hairline-subtle bg-surface p-5"><Icon icon={String(icon)} className="text-2xl text-brand-ink" /><p className="mt-3 text-xs font-semibold text-ink-tertiary">{label}</p><p className="mt-1 text-3xl font-semibold">{value}</p><p className="mt-2 text-xs text-ink-secondary">{hint}</p></article>)}</div>}

      {view === "dashboard" && <section className="min-h-0 flex-1 overflow-y-auto overscroll-contain rounded-2xl border border-hairline-subtle bg-surface p-5 [scrollbar-gutter:stable] [scrollbar-width:thin]"><div><h3 className="font-semibold">章节张力 / 质量仪表盘</h3><p className="mt-1 text-xs text-ink-tertiary">柱高为总张力，圆点显示质量门禁分。</p></div><div className="mt-6"><NovelScoreTrend rows={dashboard?.tensionCurve ?? []} /></div></section>}
      ...
      {view === "cockpit" && <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain xl:overflow-hidden">
        ...
        <section className="flex min-h-[420px] min-w-0 flex-col overflow-hidden rounded-[14px] border border-hairline-subtle bg-surface p-4 xl:min-h-0">
        ...
        <section className="flex min-h-[280px] min-w-0 flex-col overflow-hidden rounded-[14px] border border-console-ink/15 bg-console p-4 text-console-ink xl:min-h-0">
          ...
          <div ref={logScrollRef} onScroll={(event) => { const element = event.currentTarget; logFollowingRef.current = element.scrollHeight - element.scrollTop - element.clientHeight < SCROLL_FOLLOW_THRESHOLD_PX; }} className="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain pr-1 font-mono text-xs leading-5 [scrollbar-gutter:stable] [scrollbar-width:thin]">
            {logEvents.map((event) => <p key={event.id} className="break-words"><span className="text-brand-ink" title={new Date(event.createdAt).toLocaleString("zh-CN")}>[{novelRunEventTime(event.createdAt)}]</span> <span className="text-console-ink/60">{novelRunEventScope(event)}</span> {novelRunEventText(event)}</p>)}
            {logEvents.length === 0 && events.length > 0 && <p className="text-console-ink/60">正文正在流式生成，流程事件将在步骤完成后继续更新…</p>}
            {events.length === 0 && <p className="text-console-ink/60">正在加载最近运行事件…</p>}
          </div>
        </section>
        </div>
        {runs.length > 1 && <section className="mt-4 flex-none overflow-hidden rounded-[14px] border border-hairline-subtle bg-surface"><button type="button" aria-expanded={historyOpen} aria-label={historyOpen ? "收起运行历史" : "展开运行历史"} onClick={() => setHistoryOpen((value) => !value)} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left"><span><span className="block text-sm font-semibold text-ink">运行历史</span><span className="mt-0.5 block text-[10px] text-ink-tertiary">共 {runs.length} 次运行，默认收起以聚焦当前任务</span></span><span className="flex items-center gap-1 text-xs font-semibold text-brand-ink">{historyOpen ? "收起" : "展开"}<Icon icon={historyOpen ? "mdi:chevron-up" : "mdi:chevron-down"} /></span></button>{historyOpen && <div className="flex gap-2 overflow-x-auto border-t border-hairline-subtle px-4 py-3 [scrollbar-width:thin]">{runs.map((run) => <button key={run.id} type="button" onClick={() => setActiveRunId(run.id)} className={`shrink-0 rounded-lg border px-3 py-2 text-left text-xs ${activeRun?.id === run.id ? "border-brand/40 bg-brand-soft text-brand-ink" : "border-hairline-subtle"}`}><span className="block font-semibold">{run.mode === "autopilot" ? "全托管" : "辅助写作"} · {STATUS_LABELS[run.status] ?? run.status}</span><span className="mt-1 block text-ink-tertiary">{new Date(run.createdAt).toLocaleString("zh-CN")}</span></button>)}</div>}</section>}
      </div>}
    </section>
  );
}
