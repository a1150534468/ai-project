...
function EditorField({ label, value, onChange, multiline = false, type = "text", options }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean; type?: string; options?: readonly [string, string][] }) {
  return <label className="grid gap-1.5 text-xs font-semibold text-ink-secondary"><span>{label}</span>{options ? <select value={value} onChange={(event) => onChange(event.currentTarget.value)} className="h-10 rounded-xl border border-hairline bg-white px-3 text-sm font-normal outline-none focus:border-brand">{options.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select> : multiline ? <textarea value={value} onChange={(event) => onChange(event.currentTarget.value)} rows={4} className="resize-y rounded-xl border border-hairline bg-white p-3 text-sm font-normal leading-6 outline-none focus:border-brand" /> : <input value={value} onChange={(event) => onChange(event.currentTarget.value)} type={type} className="h-10 rounded-xl border border-hairline bg-white px-3 text-sm font-normal outline-none focus:border-brand" />}</label>;
}
