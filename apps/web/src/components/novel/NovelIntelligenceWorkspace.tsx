...
const EMPTY_ASSETS: NovelNarrativeAssets = { timeline: [], foreshadows: [], debts: [], events: [], causalEdges: [], facts: [] };
...
export function NovelIntelligenceWorkspace({ token, projectId, showPrompts = true }: { readonly token: string; readonly projectId: string; readonly showPrompts?: boolean }) {
  ...
  return <section className="grid gap-4">
    ...
    {tab === "timeline" && <section className="rounded-2xl border border-[#e1e6e4] bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">世界时间线</h3><p className="mt-1 text-xs text-[#7a8380]">同时记录世界内时间与对应章节，防止时序漂移。</p></div><button type="button" onClick={() => openEditor("timeline")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 添加事件</button></div><div className="relative mt-5 grid gap-3 before:absolute before:bottom-4 before:left-[76px] before:top-4 before:w-px before:bg-brand/25">{assets.timeline.map((item) => <article key={text(item.id)} className="group relative grid grid-cols-[64px_12px_minmax(0,1fr)] items-start gap-2"><span className="pt-3 text-right text-[10px] font-semibold text-brand-ink">{text(item.timeLabel) || `第${text(item.chapterNumber) || "-"}章`}</span><span className="z-10 mt-3 h-3 w-3 rounded-full border-2 border-brand bg-white" /><div className="rounded-xl border border-[#e3e8e6] p-3"><div className="flex justify-between gap-3"><div><h4 className="text-sm font-semibold">{text(item.title)}</h4><p className="mt-1 text-[10px] text-[#89928f]">对应第 {text(item.chapterNumber) || "-"} 章 · {listText(item.participants) || "无指定参与者"}</p></div><ActionButtons onEdit={() => openEditor("timeline", item)} onDelete={() => void remove("timeline", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-[#65706c]">{text(item.description)}</p></div></article>)}{!assets.timeline.length && <p className="py-14 text-center text-sm text-[#89928f]">暂无时间线事件</p>}</div></section>}

    {tab === "foreshadows" && <div className="grid gap-4 xl:grid-cols-2"><section className="rounded-2xl border border-[#e1e6e4] bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">伏笔账本</h3><p className="mt-1 text-xs text-[#7a8380]">埋设、强化、回收全程追踪。</p></div><button type="button" onClick={() => openEditor("foreshadows")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 新伏笔</button></div><div className="mt-4 grid gap-2">{assets.foreshadows.map((item) => <article key={text(item.id)} className="group rounded-xl border border-[#e3e8e6] p-3"><div className="flex justify-between gap-3"><div><span className="text-[10px] font-semibold text-brand-ink">{text(item.status)} · 第{text(item.introducedInChapterIndex) || "-"}章 → 第{text(item.expectedPayoffChapter) || "?"}章</span><h4 className="mt-1 text-sm font-semibold">{text(item.title)}</h4></div><ActionButtons onEdit={() => openEditor("foreshadows", item)} onDelete={() => void remove("foreshadows", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-[#65706c]">{text(item.description)}</p></article>)}{!assets.foreshadows.length && <p className="py-12 text-center text-sm text-[#89928f]">暂无伏笔</p>}</div></section><section className="rounded-2xl border border-[#e1e6e4] bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">叙事债务</h3><p className="mt-1 text-xs text-[#7a8380]">未回答问题、未兑现承诺和连续性风险。</p></div><button type="button" onClick={() => openEditor("narrative-debts")} className="h-9 rounded-xl border border-brand/30 px-3 text-xs font-semibold text-brand-ink">+ 新债务</button></div><div className="mt-4 grid gap-2">{assets.debts.map((item) => <article key={text(item.id)} className="group rounded-xl border border-[#e3e8e6] p-3"><div className="flex justify-between gap-3"><div><span className={`text-[10px] font-semibold ${text(item.severity) === "critical" || text(item.severity) === "high" ? "text-red-600" : "text-amber-600"}`}>{text(item.severity)} · {text(item.status)} · 到期第{text(item.dueChapter) || "?"}章</span><h4 className="mt-1 text-sm font-semibold">{text(item.title)}</h4></div><ActionButtons onEdit={() => openEditor("narrative-debts", item)} onDelete={() => void remove("narrative-debts", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-[#65706c]">{text(item.description)}</p></article>)}{!assets.debts.length && <p className="py-12 text-center text-sm text-[#89928f]">暂无叙事债务</p>}</div></section></div>}

    {tab === "props" && <section className="rounded-2xl border border-[#e1e6e4] bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">道具生命周期</h3><p className="mt-1 text-xs text-[#7a8380]">追踪持有者、位置、状态与章内变化。</p></div><button type="button" onClick={() => openEditor("props")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 新道具</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{props.map((item) => <article key={text(item.id)} className="group rounded-2xl border border-[#e3e8e6] p-4"><div className="flex justify-between gap-3"><Icon icon="mdi:treasure-chest-outline" className="text-2xl text-brand-ink" /><ActionButtons onEdit={() => openEditor("props", item)} onDelete={() => void remove("props", text(item.id), text(item.name))} /></div><h4 className="mt-2 font-semibold">{text(item.name)}</h4><p className="mt-1 text-[10px] font-semibold text-brand-ink">{text(item.status)} · {text(item.owner) || "无持有者"} · {text(item.location) || "位置未知"}</p><p className="mt-3 text-xs leading-5 text-[#65706c]">{text(item.description)}</p><div className="mt-3 grid gap-1 border-t border-[#edf0ef] pt-2">{rows(item.events).slice(0, 4).map((event) => <p key={text(event.id)} className="text-[10px] text-[#7a8380]">第{text(event.chapterNumber) || "-"}章 · {text(event.eventType)} · {text(event.description)}</p>)}</div></article>)}{!props.length && <p className="py-14 text-center text-sm text-[#89928f] sm:col-span-2 xl:col-span-3">暂无道具</p>}</div></section>}
