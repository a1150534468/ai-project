...
function EditorField({ label, value, onChange, multiline = false, type = "text", options }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean; type?: string; options?: readonly [string, string][] }) {
  return <label className="grid gap-1.5 text-xs font-semibold text-ink-secondary"><span>{label}</span>{options ? <select value={value} onChange={(event) => onChange(event.currentTarget.value)} className="h-10 rounded-xl border border-hairline bg-surface px-3 text-sm font-normal outline-none focus:border-brand">{options.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select> : multiline ? <textarea value={value} onChange={(event) => onChange(event.currentTarget.value)} rows={4} className="resize-y rounded-xl border border-hairline bg-surface p-3 text-sm font-normal leading-6 outline-none focus:border-brand" /> : <input value={value} onChange={(event) => onChange(event.currentTarget.value)} type={type} className="h-10 rounded-xl border border-hairline bg-surface px-3 text-sm font-normal outline-none focus:border-brand" />}</label>;
}
...
function EditorDialog({ editor, structure, busy, error, onChange, onClose, onSave }: { editor: EditorState; structure: NovelStructureNode[]; busy: boolean; error: string; onChange: (key: string, value: string) => void; onClose: () => void; onSave: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-scrim/55 p-3 backdrop-blur-sm"><section className="max-h-[92dvh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-surface-subtle shadow-2xl [scrollbar-width:thin]"><header className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline-subtle bg-surface px-5 py-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-brand-ink">Narrative editor</p><h3 className="mt-1 text-lg font-semibold text-ink">{editor.id ? "编辑" : "新建"}{EDITOR_TITLES[editor.kind]}</h3></div><button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl border border-hairline"><Icon icon="mdi:close" /></button></header><div className="grid gap-4 p-5 sm:grid-cols-2">
    {editor.kind === "structure" && <><EditorField label="节点类型" value={v.nodeType ?? "chapter"} onChange={(value) => onChange("nodeType", value)} options={[["book", "全书"], ["volume", "卷"], ["act", "幕"], ["chapter", "章"]]} /><label className="grid gap-1.5 text-xs font-semibold text-ink-secondary"><span>父节点</span><select value={v.parentId ?? ""} onChange={(event) => onChange("parentId", event.currentTarget.value)} className="h-10 rounded-xl border border-hairline bg-surface px-3 text-sm font-normal"><option value="">根节点</option>{structure.filter((node) => node.id !== editor.id && node.nodeType !== "chapter").map((node) => <option key={node.id} value={node.id}>{node.nodeType} · {node.title}</option>)}</select></label><EditorField label="标题" value={v.title ?? ""} onChange={(value) => onChange("title", value)} /><EditorField label="排序编号" type="number" value={v.number ?? "1"} onChange={(value) => onChange("number", value)} /><EditorField label="开始章节" type="number" value={v.startChapter ?? ""} onChange={(value) => onChange("startChapter", value)} /><EditorField label="结束章节" type="number" value={v.endChapter ?? ""} onChange={(value) => onChange("endChapter", value)} /><div className="sm:col-span-2"><EditorField label="节点说明" value={v.description ?? ""} onChange={(value) => onChange("description", value)} multiline /></div><div className="sm:col-span-2"><EditorField label="规划大纲" value={v.outline ?? ""} onChange={(value) => onChange("outline", value)} multiline /></div></>}
  ...
  </div>{error && <p className="mx-5 mb-3 rounded-xl bg-danger/10 px-3 py-2 text-xs text-danger-ink">{error}</p>}<footer className="sticky bottom-0 flex justify-end gap-2 border-t border-hairline-subtle bg-surface px-5 py-4"><button type="button" onClick={onClose} className="h-10 rounded-xl border border-hairline px-4 text-xs font-semibold">取消</button><button type="button" onClick={onSave} disabled={busy} className="h-10 rounded-xl bg-brand px-5 text-xs font-semibold text-white disabled:opacity-50">{busy ? "保存中" : "保存"}</button></footer></section></div>;
}

function ActionButtons({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  return <div className="flex gap-1 opacity-100"><button type="button" onClick={onEdit} className="grid h-7 w-7 place-items-center rounded-lg text-ink-secondary" aria-label="编辑"><Icon icon="mdi:pencil-outline" /></button><button type="button" onClick={onDelete} className="grid h-7 w-7 place-items-center rounded-lg text-ink-tertiary" aria-label="删除"><Icon icon="mdi:trash-can-outline" /></button></div>;
}
...
export function NovelIntelligenceWorkspace({ token, projectId, showPrompts = true }: { readonly token: string; readonly projectId: string; readonly showPrompts?: boolean }) {
  ...
  return <section className="grid gap-4">
    ...
    {tab === "overview" && <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{cards.map(([label, value, hint, icon]) => <article key={label} className="rounded-2xl border border-hairline-subtle bg-surface p-4"><Icon icon={icon} className="text-xl text-brand-ink" /><p className="mt-3 text-xs font-semibold text-ink-tertiary">{label}</p><p className="mt-1 text-2xl font-semibold text-ink">{value}</p><p className="mt-1 text-xs text-ink-secondary">{hint}</p></article>)}</div><section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><h3 className="text-sm font-semibold">张力与质量趋势</h3><div className="mt-4"><NovelScoreTrend compact rows={dashboard?.tensionCurve ?? []} /></div></section></>}

    {tab === "structure" && <section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">部 · 卷 · 幕 · 章故事树</h3><p className="mt-1 text-xs text-ink-tertiary">编号决定同级顺序；编辑节点可调整父级和章节范围。</p></div><button type="button" onClick={() => openEditor("structure")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white"><Icon icon="mdi:plus" className="mr-1 inline" />新建节点</button></div><div className="mt-4 grid gap-2">{structure.map((node) => <article key={node.id} className="group flex items-start gap-3 rounded-xl border border-hairline-subtle p-3" style={{ marginLeft: node.nodeType === "volume" ? 12 : node.nodeType === "act" ? 28 : node.nodeType === "chapter" ? 44 : 0 }}><span className="mt-0.5 rounded-md bg-surface-muted px-2 py-1 text-[10px] font-bold uppercase text-brand-ink">{node.nodeType}</span><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{node.number}. {node.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-ink-secondary">{node.outline || node.description || "暂无规划"}</p></div><ActionButtons onEdit={() => openEditor("structure", node as unknown as Record<string, unknown>)} onDelete={() => void remove("structure", node.id, node.title)} /></article>)}{!structure.length && <p className="py-14 text-center text-sm text-ink-tertiary">暂无结构节点</p>}</div></section>}

    {tab === "storylines" && <section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">主线 · 支线 · 暗线</h3><p className="mt-1 text-xs text-ink-tertiary">里程碑定义各故事线在具体章节的汇流与兑现。</p></div><button type="button" onClick={() => openEditor("storylines")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white"><Icon icon="mdi:plus" className="mr-1 inline" />新建故事线</button></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{storylines.map((line) => <article key={text(line.id)} className="group rounded-2xl border border-hairline-subtle p-4"><div className="flex items-start justify-between gap-3"><div><span className="rounded-full bg-brand-soft px-2 py-1 text-[10px] font-semibold text-brand-ink">{({ main: "主线", subplot: "支线", hidden: "暗线" } as Record<string, string>)[text(line.storylineType)] ?? text(line.storylineType)}</span><h4 className="mt-2 font-semibold">{text(line.title)}</h4></div><ActionButtons onEdit={() => openEditor("storylines", line)} onDelete={() => void remove("storylines", text(line.id), text(line.title))} /></div><p className="mt-3 text-xs leading-5 text-ink-secondary">目标：{text(line.goal) || "待补充"}</p><p className="mt-1 text-xs leading-5 text-ink-secondary">冲突：{text(line.conflict) || "待补充"}</p><div className="mt-4 border-t border-hairline-subtle pt-3"><div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-wider text-ink-tertiary">里程碑</span><button type="button" onClick={() => openEditor("milestone", {}, text(line.id))} className="text-xs font-semibold text-brand-ink">+ 添加</button></div><div className="mt-2 grid gap-2">{rows(line.milestones).map((milestone) => <div key={text(milestone.id)} className="group/m flex items-center gap-2 rounded-lg bg-surface-muted px-3 py-2"><span className="text-[10px] font-bold text-brand-ink">第{text(milestone.chapterNumber)}章</span><span className="min-w-0 flex-1 truncate text-xs">{text(milestone.title)}</span><ActionButtons onEdit={() => openEditor("milestone", milestone, text(line.id))} onDelete={() => void remove("storyline-milestones", text(milestone.id), text(milestone.title))} /></div>)}</div></div></article>)}{!storylines.length && <p className="py-14 text-center text-sm text-ink-tertiary lg:col-span-2">暂无故事线</p>}</div></section>}

    {tab === "timeline" && <section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">世界时间线</h3><p className="mt-1 text-xs text-ink-tertiary">同时记录世界内时间与对应章节，防止时序漂移。</p></div><div className="flex gap-2"><button type="button" onClick={() => void backfillContinuity()} disabled={backfilling} className="flex h-9 items-center gap-1.5 rounded-xl border border-brand/30 px-3 text-xs font-semibold text-brand-ink disabled:opacity-50"><Icon icon={backfilling ? "mdi:loading" : "mdi:text-search"} className={backfilling ? "animate-spin" : ""} />{backfilling ? "扫描中" : "从正文补齐"}</button><button type="button" onClick={() => openEditor("timeline")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 添加事件</button></div></div><div className="relative mt-5 grid gap-3 before:absolute before:bottom-4 before:left-[76px] before:top-4 before:w-px before:bg-brand/25">{assets.timeline.map((item) => <article key={text(item.id)} className="group relative grid grid-cols-[64px_12px_minmax(0,1fr)] items-start gap-2"><span className="pt-3 text-right text-[10px] font-semibold text-brand-ink">{text(item.timeLabel) || `第${text(item.chapterNumber) || "-"}章`}</span><span className="z-10 mt-3 h-3 w-3 rounded-full border-2 border-brand bg-surface" /><div className="rounded-xl border border-hairline-subtle p-3"><div className="flex justify-between gap-3"><div><h4 className="text-sm font-semibold">{text(item.title)}</h4><p className="mt-1 text-[10px] text-ink-tertiary">对应第 {text(item.chapterNumber) || "-"} 章 · {listText(item.participants) || "无指定参与者"}</p></div><ActionButtons onEdit={() => openEditor("timeline", item)} onDelete={() => void remove("timeline", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-ink-secondary">{text(item.description)}</p></div></article>)}{!assets.timeline.length && <p className="py-14 text-center text-sm text-ink-tertiary">暂无时间线事件，可从现有章节正文自动补齐</p>}</div></section>}

    {tab === "foreshadows" && <div className="grid gap-4 xl:grid-cols-2"><section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold">伏笔账本</h3><p className="mt-1 text-xs text-ink-tertiary">埋设、强化、回收全程追踪。</p></div><div className="flex gap-2"><button type="button" onClick={() => void backfillContinuity()} disabled={backfilling} className="h-9 rounded-xl border border-brand/30 px-3 text-xs font-semibold text-brand-ink disabled:opacity-50">{backfilling ? "重建中" : "从正文重建"}</button><button type="button" onClick={() => openEditor("foreshadows")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 新伏笔</button></div></div><div className="mt-4 grid gap-2">{assets.foreshadows.map((item) => { const overdue = ["open", "hinted"].includes(text(item.status)) && Number(item.expectedPayoffChapter) > 0 && Number(item.expectedPayoffChapter) < (dashboard?.stats.chapters ?? 0); return <article key={text(item.id)} className="group rounded-xl border border-hairline-subtle p-3"><div className="flex justify-between gap-3"><div><span className={`text-[10px] font-semibold ${overdue ? "text-danger-ink" : "text-brand-ink"}`}>{overdue ? "overdue" : text(item.status)} · 第{text(item.introducedInChapterIndex) || "-"}章 → 第{text(item.expectedPayoffChapter) || "?"}章</span><h4 className="mt-1 text-sm font-semibold">{text(item.title)}</h4></div><ActionButtons onEdit={() => openEditor("foreshadows", item)} onDelete={() => void remove("foreshadows", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-ink-secondary">{text(item.description)}</p><div className="mt-2 grid gap-1 border-t border-hairline-subtle pt-2">{rows(item.events).map((event) => <p key={text(event.id)} className="text-[10px] text-ink-tertiary">第{text(event.chapterIndex)}章 · {({ planted: "埋设", reinforced: "强化", paidOff: "回收" } as Record<string, string>)[text(event.action)] ?? text(event.action)} · {text(event.evidence)}</p>)}</div></article>; })}{!assets.foreshadows.length && <p className="py-12 text-center text-sm text-ink-tertiary">暂无伏笔</p>}</div></section><section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">叙事债务</h3><p className="mt-1 text-xs text-ink-tertiary">未回答问题、未兑现承诺和连续性风险。</p></div><button type="button" onClick={() => openEditor("narrative-debts")} className="h-9 rounded-xl border border-brand/30 px-3 text-xs font-semibold text-brand-ink">+ 新债务</button></div><div className="mt-4 grid gap-2">{assets.debts.map((item) => { const overdue = text(item.status) === "open" && Number(item.dueChapter) > 0 && Number(item.dueChapter) < (dashboard?.stats.chapters ?? 0); return <article key={text(item.id)} className="group rounded-xl border border-hairline-subtle p-3"><div className="flex justify-between gap-3"><div><span className={`text-[10px] font-semibold ${overdue || text(item.severity) === "critical" || text(item.severity) === "high" ? "text-danger-ink" : "text-warning-ink"}`}>{text(item.severity)} · {overdue ? "overdue" : text(item.status)} · 到期第{text(item.dueChapter) || "?"}章</span><h4 className="mt-1 text-sm font-semibold">{text(item.title)}</h4></div><ActionButtons onEdit={() => openEditor("narrative-debts", item)} onDelete={() => void remove("narrative-debts", text(item.id), text(item.title))} /></div><p className="mt-2 text-xs leading-5 text-ink-secondary">{text(item.description)}</p>{text(item.resolutionEvidence) && <p className="mt-2 rounded-lg bg-brand-soft p-2 text-[10px] text-brand-ink">第{text(item.resolvedInChapter)}章解决：{text(item.resolutionEvidence)}</p>}</article>; })}{!assets.debts.length && <p className="py-12 text-center text-sm text-ink-tertiary">暂无叙事债务</p>}</div></section></div>}

    {tab === "props" && <section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">道具生命周期</h3><p className="mt-1 text-xs text-ink-tertiary">追踪持有者、位置、状态与章内变化。</p></div><div className="flex gap-2"><button type="button" onClick={() => void backfillContinuity()} disabled={backfilling} className="flex h-9 items-center gap-1.5 rounded-xl border border-brand/30 px-3 text-xs font-semibold text-brand-ink disabled:opacity-50"><Icon icon={backfilling ? "mdi:loading" : "mdi:text-search"} className={backfilling ? "animate-spin" : ""} />{backfilling ? "扫描中" : "从正文补齐"}</button><button type="button" onClick={() => openEditor("props")} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white">+ 新道具</button></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{props.map((item) => <article key={text(item.id)} className="group rounded-2xl border border-hairline-subtle p-4"><div className="flex justify-between gap-3"><Icon icon="mdi:treasure-chest-outline" className="text-2xl text-brand-ink" /><ActionButtons onEdit={() => openEditor("props", item)} onDelete={() => void remove("props", text(item.id), text(item.name))} /></div><h4 className="mt-2 font-semibold">{text(item.name)}</h4><p className="mt-1 text-[10px] font-semibold text-brand-ink">{PROP_STATUS_LABELS[text(item.status)] ?? text(item.status)} · {text(item.owner) || "无持有者"} · {text(item.location) || "位置未知"}</p><p className="mt-3 text-xs leading-5 text-ink-secondary">{text(item.description)}</p><div className="mt-3 grid gap-1 border-t border-hairline-subtle pt-2">{rows(item.events).slice(0, 4).map((event) => <p key={text(event.id)} className="text-[10px] text-ink-tertiary">第{text(event.chapterNumber) || "-"}章 · {PROP_EVENT_LABELS[text(event.eventType)] ?? text(event.eventType)} · {text(event.description)}</p>)}</div></article>)}{!props.length && <p className="py-14 text-center text-sm text-ink-tertiary sm:col-span-2 xl:col-span-3">暂无道具，可从现有章节正文自动补齐</p>}</div></section>}

    {tab === "causal" && <div className="grid gap-4 xl:grid-cols-2"><section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><h3 className="font-semibold">知识图谱事实</h3><div className="mt-4 grid gap-2">{assets.facts.slice(0, 80).map((fact) => <div key={text(fact.id)} className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 rounded-xl bg-surface-muted p-3 text-xs"><span className="font-semibold">{text(fact.subject)}</span><span className="rounded-full bg-surface px-2 py-1 text-[10px] text-brand-ink">{text(fact.predicate)}</span><span className="text-right">{text(fact.object)}</span></div>)}{!assets.facts.length && <p className="py-12 text-center text-sm text-ink-tertiary">章后分析会生成稳定事实</p>}</div></section><section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><h3 className="font-semibold">事件因果链</h3><div className="mt-4 grid gap-3">{assets.events.map((event) => <article key={text(event.id)} className="rounded-xl border border-hairline-subtle p-3"><div className="flex items-center gap-2"><span className="rounded bg-brand-soft px-2 py-1 text-[10px] font-semibold text-brand-ink">第{text(event.chapterNumber)}章</span><h4 className="text-sm font-semibold">{text(event.title)}</h4></div><p className="mt-2 text-xs text-ink-secondary">{text(event.description)}</p><div className="mt-2 flex flex-wrap gap-1">{assets.causalEdges.filter((edge) => text(edge.fromEventId) === text(event.id)).map((edge) => <span key={text(edge.id)} className="rounded-full bg-surface-muted px-2 py-1 text-[10px] text-ink-secondary">{text(edge.relationType)} → {text(edge.toEventId).slice(-6)}</span>)}</div></article>)}{!assets.events.length && <p className="py-12 text-center text-sm text-ink-tertiary">章后同步会生成事件与因果关系</p>}</div></section></div>}

    {tab === "checkpoints" && <section className="rounded-2xl border border-hairline-subtle bg-surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">检查点与世界线</h3><p className="mt-1 text-xs text-ink-tertiary">每次生成前自动保存，也可手动创建、回滚和分支。</p></div><button type="button" onClick={() => void createCheckpoint()} disabled={busy} className="h-9 rounded-xl bg-brand px-3 text-xs font-semibold text-white disabled:opacity-50">创建检查点</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{checkpoints.map((item) => <article key={text(item.id)} className="rounded-2xl border border-hairline-subtle p-4"><div className="flex justify-between"><h4 className="text-sm font-semibold">{text(item.label) || "检查点"}</h4>{Boolean(item.isHead) && <span className="rounded-full bg-brand-soft px-2 py-1 text-[9px] font-semibold text-brand-ink">HEAD</span>}</div><p className="mt-2 text-xs text-ink-tertiary">{text(item.branchName)} · 第{text(item.chapterNumber) || "-"}章</p><div className="mt-4 flex gap-3">{!item.isHead && <button type="button" onClick={() => void rollback(text(item.id))} className="text-xs font-semibold text-danger-ink">回滚</button>}<button type="button" onClick={() => void branch(text(item.id))} className="text-xs font-semibold text-brand-ink">从这里分支</button></div></article>)}</div></section>}
