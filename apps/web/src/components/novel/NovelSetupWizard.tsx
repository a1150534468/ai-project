  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#101615]/55 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="新书设置向导">
      <section className="grid max-h-[94dvh] w-full max-w-[1120px] grid-rows-[auto_auto_minmax(0,1fr)_auto] overflow-hidden rounded-3xl bg-[#f5f7f6] shadow-2xl">
        ...
        <nav className="flex gap-1 overflow-x-auto border-b border-[#e1e5e3] bg-white px-4 py-3 [scrollbar-width:none]">{STEPS.map((item, index) => { const active = index + 1 === step; const unlockedStage = Math.max(step, setup?.project.setupStage ?? project.setupStage ?? 1); const complete = index + 1 < (setup?.project.setupStage ?? project.setupStage ?? 1) || Boolean(setup?.project.setupCompleted && index === 4); const unlocked = index + 1 <= unlockedStage; return <button key={item.kind} type="button" disabled={!unlocked} aria-current={active ? "step" : undefined} onClick={() => setStep(index + 1)} className={`flex min-w-[170px] flex-1 items-center gap-3 rounded-xl px-3 py-2 text-left transition disabled:cursor-not-allowed ${active ? "bg-brand-soft ring-1 ring-brand/25" : complete ? "bg-[#f5f7f6]" : "opacity-55"}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-sm ${active ? "bg-brand text-white" : complete ? "bg-emerald-100 text-emerald-700" : "bg-[#edf0ef] text-[#7a8380]"}`}><Icon icon={complete ? "mdi:check" : item.icon} /></span><span><span className="block text-xs font-semibold text-[#303936]">{index + 1}. {item.title}</span><span className="mt-0.5 block text-[10px] text-[#818986]">{item.description}</span></span></button>; })}</nav>
