    ...
    <div className="grid min-h-96 place-items-center">
      <div className="w-full max-w-2xl">
        <div className="text-center">
          ...
          <h3 className="mt-5 text-lg font-semibold text-[#25302d]">正在生成{title}</h3>
        ...
        <section className="mt-5 rounded-2xl border border-[#dfe6e3] bg-white p-4 text-left shadow-sm">
          <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-brand-ink">{stageLabel}</p><p className="mt-1 text-xs text-[#68716e]" aria-live="polite">{task.progressMessage || "等待任务状态更新"}</p></div><strong className="text-lg tabular-nums text-[#26302d]">{progressPercent}%</strong></div>
          ...
          {preview && <div className="mt-4 rounded-xl bg-[#111816] p-3 text-[#d9ebe5]"><div className="flex items-center justify-between gap-3"><p className="text-[11px] font-semibold text-[#8fe1cd]">AI 实时输出</p><p className="text-[10px] text-[#7f9991]">实际生成内容，不是隐藏推理</p></div><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-all font-mono text-[11px] leading-5 [scrollbar-width:thin]">{preview}</pre></div>}
        </section>
      </div>
    </div>
  );
