...
function Preview({ setup, kind }: { readonly setup: NovelSetupPayload; readonly kind: NovelSetupKind }) {
  if (kind === "bible") {
    ...
    return <div className="grid gap-4"><section className="rounded-2xl border border-hairline-subtle bg-white p-4"><h4 className="flex items-center gap-2 text-sm font-semibold text-ink"><Icon icon="mdi:feather" className="text-brand-ink" />文风公约</h4><div className="mt-3 grid gap-2 sm:grid-cols-2">{styles.map((item, index) => <div key={text(item.id) || index} className="rounded-xl bg-surface-subtle p-3"><p className="text-xs font-semibold text-brand-ink">{text(item.title) || text(item.category)}</p><p className="mt-1 whitespace-pre-wrap text-xs leading-5 text-ink-secondary">{text(item.content)}</p></div>)}</div></section><section className="rounded-2xl border border-hairline-subtle bg-white p-4"><h4 className="flex items-center gap-2 text-sm font-semibold text-ink"><Icon icon="mdi:earth" className="text-brand-ink" />五维世界观</h4><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{dimensions.map((item, index) => <div key={text(item.id) || index} className="rounded-xl border border-hairline-subtle p-3"><p className="text-xs font-semibold text-ink">{text(item.title)}</p><p className="mt-1 line-clamp-5 text-xs leading-5 text-ink-secondary">{text(item.summary) || JSON.stringify(item.details)}</p></div>)}</div></section></div>;
  }
  if (kind === "characters") return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{setup.characters.map((item, index) => <article key={text(item.id) || index} className="rounded-2xl border border-hairline-subtle bg-white p-4"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-full bg-brand-soft font-semibold text-brand-ink">{text(item.name).slice(0, 1) || "?"}</div><div><h4 className="text-sm font-semibold text-ink">{text(item.name) || "未命名人物"}</h4><p className="text-xs text-brand-ink">{text(item.role) || "人物"}</p></div></div><p className="mt-3 line-clamp-4 text-xs leading-5 text-ink-secondary">{text(item.coreMotivation) || text(item.description) || "等待补充人物锚点"}</p></article>)}</div>;
  if (kind === "locations") return <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{setup.locations.map((item, index) => <article key={text(item.id) || index} className="rounded-2xl border border-hairline-subtle bg-white p-4"><Icon icon="mdi:map-marker-radius-outline" className="text-xl text-brand-ink" /><h4 className="mt-2 text-sm font-semibold text-ink">{text(item.name) || "未命名地点"}</h4><p className="mt-2 line-clamp-4 text-xs leading-5 text-ink-secondary">{text(item.description)}</p><p className="mt-2 text-[11px] text-ink-tertiary">规则：{text(item.rules) || "待补充"}</p></article>)}</div>;
  return <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><section className="rounded-2xl border border-hairline-subtle bg-white p-4"><h4 className="text-sm font-semibold text-ink">部 · 卷 · 幕 · 章</h4><div className="mt-3 grid max-h-96 gap-2 overflow-y-auto [scrollbar-width:thin]">{setup.structure.map((node) => <div key={node.id} className="rounded-xl border border-hairline-subtle px-3 py-2" style={{ marginLeft: node.nodeType === "volume" ? 8 : node.nodeType === "act" ? 20 : node.nodeType === "chapter" ? 32 : 0 }}><p className="text-xs font-semibold text-ink">{node.nodeType} · {node.title}</p><p className="mt-1 line-clamp-2 text-[11px] text-ink-tertiary">{node.outline || node.description}</p></div>)}</div></section><section className="rounded-2xl border border-hairline-subtle bg-white p-4"><h4 className="text-sm font-semibold text-ink">故事线</h4><div className="mt-3 grid gap-2">{setup.storylines.map((item, index) => <div key={text(item.id) || index} className="rounded-xl bg-surface-subtle p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold text-ink">{text(item.title)}</p><span className="text-[10px] font-semibold text-brand-ink">{text(item.storylineType)}</span></div><p className="mt-1 line-clamp-3 text-[11px] leading-5 text-ink-secondary">{text(item.goal) || text(item.conflict)}</p></div>)}</div></section></div>;
}
    ...
    <div className="grid min-h-96 place-items-center">
      <div className="w-full max-w-2xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-ink-secondary">关闭向导不会中断，重新打开后会继续显示真实任务进度。</p>
        </div>
        <section className="mt-5 rounded-2xl border border-hairline-subtle bg-white p-4 text-left shadow-sm">
          <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-brand-ink">{stageLabel}</p><p className="mt-1 text-xs text-ink-secondary" aria-live="polite">{task.progressMessage || "等待任务状态更新"}</p></div><strong className="text-lg tabular-nums text-ink">{progressPercent}%</strong></div>
          <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-surface-muted" role="progressbar" aria-label={`${title}生成进度`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progressPercent}><div className="h-full rounded-full bg-brand transition-[width] duration-700 ease-out" style={{ width: `${progressPercent}%` }} /></div>
          <div className="mt-2 flex items-center justify-between text-[11px] text-ink-tertiary"><span>按真实执行阶段与已接收输出计算</span><span className="tabular-nums">已接收 {task.streamedChars.toLocaleString("zh-CN")} 字</span></div>
          {preview && <div className="mt-4 rounded-xl bg-console p-3 text-console-ink"><div className="flex items-center justify-between gap-3"><p className="text-[11px] font-semibold text-brand-ink">AI 实时输出</p><p className="text-[10px] text-console-ink/70">实际生成内容，不是隐藏推理</p></div><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-all font-mono text-[11px] leading-5 [scrollbar-width:thin]">{preview}</pre></div>}
        </section>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-scrim/55 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="新书设置向导">
      <section className="grid max-h-[94dvh] w-full max-w-[1120px] grid-rows-[auto_auto_minmax(0,1fr)_auto] overflow-hidden rounded-3xl bg-surface-muted shadow-2xl">
        ...
        <main className="min-h-0 overflow-y-auto p-4 sm:p-6 [scrollbar-width:thin]">
          ...
          {current.kind === "complete" ? isProjectSettings ? <div className="grid gap-5"><div className="rounded-2xl border border-hairline-subtle bg-brand-soft p-5 text-left"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-2xl text-brand-ink shadow-sm"><Icon icon="mdi:book-cog-outline" /></div><div><h3 className="text-base font-semibold text-ink">作品设置已生效</h3><p className="mt-1 text-xs text-ink-secondary">Bible、人物、地点和故事树均可从上方步骤继续查看和修改。</p></div></div></div><ProjectFileActions token={token} projectId={project.id} onImported={onProjectChanged} /></div> : <div className="grid min-h-96 place-items-center text-center"><div><div className="mx-auto grid h-20 w-20 place-items-center rounded-3xl bg-brand-soft text-4xl text-brand-ink"><Icon icon="mdi:rocket-launch-outline" /></div><h3 className="mt-5 text-xl font-semibold text-ink">叙事基座已经就绪</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-ink-secondary">Bible、人物、地点和故事树已写入作品数据。进入工作台后，可以从第一章的执行剧本开始辅助创作，也可以启动全托管。</p><button type="button" onClick={() => void complete()} disabled={Boolean(busy)} className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-brand px-6 text-sm font-semibold text-white disabled:opacity-50"><Icon icon={busy ? "mdi:loading" : "mdi:arrow-right"} className={busy ? "animate-spin" : ""} />进入作品工作台</button></div></div> : generating && activeTask ? <GenerationProgress title={current.title} icon={current.icon} task={activeTask} progressPercent={progressPercent} stageLabel={progressStageLabel} /> : failedTask ? <GenerationFailure title={current.title} icon={current.icon} error={failedTask.error} busy={Boolean(busy)} onRetry={() => void generate()} /> : generated && setup && !editing ? <Preview setup={setup} kind={current.kind} /> : editing ? <div className="grid gap-3"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold text-ink">编辑本步结构化数据</h3><p className="mt-1 text-xs text-ink-tertiary">保存后直接覆盖本步骤的作品数据，不经过旧阶段文本块。</p></div><button type="button" onClick={() => setEditing(false)} className="h-9 rounded-xl border border-hairline bg-white px-3 text-xs font-semibold">返回预览</button></div><textarea value={editor} onChange={(event) => setEditor(event.currentTarget.value)} rows={24} className="w-full resize-y rounded-2xl border border-hairline bg-console p-4 font-mono text-xs leading-6 text-console-ink outline-none focus:border-brand" /></div> : <div className="grid min-h-96 place-items-center text-center"><div><div className="mx-auto grid h-20 w-20 place-items-center rounded-3xl bg-brand-soft text-4xl text-brand-ink"><Icon icon={current.icon} /></div><h3 className="mt-5 text-lg font-semibold text-ink">准备生成{current.title}</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-ink-secondary">AI 会基于已锁定梗概与前序确认结果生成结构化资料，完成后可逐项检查和修改。</p><button type="button" onClick={() => void generate()} disabled={Boolean(busy)} className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl bg-brand px-5 text-sm font-semibold text-white disabled:opacity-50"><Icon icon={busy ? "mdi:loading" : "mdi:creation-outline"} className={busy ? "animate-spin" : ""} />开始生成</button></div></div>}
        </main>
          {current.kind !== "complete" && <footer className="flex flex-col gap-3 border-t border-hairline-subtle bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-ink-tertiary">{generated ? setup?.project.setupCompleted ? "作品已启用；可修改结构化资料，AI 重生成请在对应工作区执行。" : "已生成，可逐项修改并确认进入下一步。" : "生成结果会直接保存到新小说领域模型。"}</p><div className="flex flex-wrap gap-2">{generated && <><button type="button" onClick={() => setEditing((value) => !value)} className="h-10 rounded-xl border border-hairline px-4 text-xs font-semibold text-ink-secondary"><Icon icon="mdi:pencil-outline" className="mr-1 inline" />{editing ? "预览" : "修改"}</button>{!setup?.project.setupCompleted && <button type="button" onClick={() => void generate()} disabled={Boolean(busy)} className="h-10 rounded-xl border border-brand/30 px-4 text-xs font-semibold text-brand-ink">重新生成</button>}{(!setup?.project.setupCompleted || editing) && <button type="button" onClick={() => void saveAndNext()} disabled={Boolean(busy)} className="h-10 rounded-xl bg-brand px-5 text-xs font-semibold text-white disabled:opacity-50">{busy === "save" ? "保存中" : setup?.project.setupCompleted ? "保存修改" : "确认并继续"}<Icon icon="mdi:arrow-right" className="ml-1 inline" /></button>}</>}</div></footer>}
      </section>
    </div>
  );
}
