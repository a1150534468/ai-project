    ...
    <section className="grid min-h-full w-full gap-8 px-4 pb-32 pt-6 lg:px-7">
      ...
      <section className="grid gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3"><h2 className="text-xl font-semibold text-ink">我的书目</h2><span className="rounded-full bg-[#eaf1ef] px-2.5 py-1 text-xs font-semibold text-[#62706c]">{visible.length} 本</span></div>
                ...
                <article key={project.id} className={`group relative grid min-h-[218px] w-[310px] flex-none snap-start overflow-hidden rounded-2xl border bg-white shadow-sm transition ${checked ? "border-brand ring-2 ring-brand/10" : "border-[#e1e5e3] "}`}>
                  ...
                  <div className="grid content-between p-5">
                    <div>
                      ...
                      <button type="button" onClick={() => onOpenProject(project.id)} className="mt-4 block w-full text-left"><h3 className="truncate text-lg font-semibold text-ink">{project.title}</h3><p className="mt-2 truncate text-sm text-[#6d7673]">{project.genre || "未设置题材"}</p></button>
                    </div>
