...
export function WorkflowFlyout({ items, isActive, onSelect }: WorkflowFlyoutProps) {
  ...
  return (
    <div className="min-w-[152px] rounded-xl border border-hairline-subtle bg-surface p-1.5 shadow-[0_8px_28px_rgba(0,0,0,0.13)]">
      <p className="px-2 pb-1.5 pt-1 text-[10px] font-medium uppercase tracking-wide text-ink-tertiary">工作流</p>
      {items.map((sub, idx) => {
          ...
          <motion.button
            ...
            className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs transition-colors ${
              active ? "bg-brand-soft font-semibold text-brand-ink"
                : sub.developing ? "cursor-not-allowed text-ink-tertiary" : "text-ink-secondary "
            }`}
          >
