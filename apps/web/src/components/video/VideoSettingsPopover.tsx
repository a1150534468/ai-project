  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" aria-label="视频设置" aria-expanded={open} onClick={toggle}
        className={`flex w-full items-center gap-2 rounded-[10px] border bg-white px-3 py-2.5 text-left transition ${open ? "border-[#1d1d1f]" : "border-[#e8e8ed] hover:border-[#d2d2d7]"}`}>
              ...
              {ASPECT_OPTIONS.map((a) => {
                ...
                return (
                  <button key={a} type="button" onClick={() => props.onAspectRatioChange(a)}
                    className={`rounded-[8px] border px-1 py-2 text-[11px] transition ${on ? "border-[#1d1d1f] font-semibold text-[#1d1d1f]" : "border-[#e8e8ed] text-[#8a8a8f] hover:border-[#d2d2d7]"}`}>
                    {a}
                  </button>
                );
              })}
              ...
              {MODEL_RESOLUTION_OPTIONS[model].map((r) => {
                ...
                return (
                  <button key={r} type="button" onClick={() => props.onResolutionChange(r)}
                    className={`rounded-[8px] border px-3 py-2 text-[12px] transition ${on ? "border-[#1d1d1f] font-semibold text-[#1d1d1f]" : "border-[#e8e8ed] text-[#8a8a8f] hover:border-[#d2d2d7]"}`}>
                    {RESOLUTION_LABELS[r]}
                  </button>
                );
              })}
