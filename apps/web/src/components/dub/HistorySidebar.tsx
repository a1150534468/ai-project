...
const STAGE_LABEL: Record<string, { text: string; cls: string }> = {
  draft: { text: "草稿", cls: "bg-gray-100 text-[#8a8a8f]" },
  analyzed: { text: "已拆解", cls: "bg-gray-100 text-[#8a8a8f]" },
  scripted: { text: "已洗稿", cls: "bg-gray-100 text-[#8a8a8f]" },
...
export function HistorySidebar({ projects, activeId, onSelect, onCreate, onDelete }: HistorySidebarProps) {
  return (
    <aside className="flex w-60 shrink-0 flex-col gap-3 border-r border-gray-100 pr-4">
      ...
      <p className="px-1 text-[11.5px] font-medium text-[#b6b6bd]">历史任务</p>

      <div className="flex-1 space-y-1.5 overflow-y-auto">
        {projects.length === 0 && <p className="px-1 text-[12px] text-[#c7c7cc]">还没有任务</p>}
        {projects.map((p) => {
            ...
            >
              <button onClick={() => onSelect(p.id)} className="w-full text-left">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="truncate text-[13px] font-medium text-[#1d1d1f]">{p.title}</span>
                </div>
                <div className="mt-1 flex items-center gap-1.5">
                  <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${badge.cls}`}>{badge.text}</span>
                  <span className="text-[10.5px] text-[#c7c7cc]">{new Date(p.createdAt).toLocaleDateString()}</span>
                </div>
              </button>
              <button
