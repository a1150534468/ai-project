...
function renderPicker(overrides: Partial<{
  selectedTheme: "auto" | "literary";
  selectedThemeColor: string;
}> = {}) {
  return renderToStaticMarkup(
    <ArticleWorkflowThemePicker
      selectedTheme={overrides.selectedTheme ?? "auto"}
      selectedThemeColor={overrides.selectedThemeColor ?? ""}
      galleryMode="collage"
      onThemeChange={() => undefined}
      onThemeColorChange={() => undefined}
      onGalleryModeChange={() => undefined}
    />,
  );
}
...
describe("ArticleWorkflowThemePicker", () => {
  ...
  it("shows the color picker seeded with the theme primary color", () => {
    const html = renderPicker({ selectedTheme: "literary" });
    expect(html).toContain("自定义主色");
    expect(html).toContain("#9c4b3f");
  });

  it("prefers the user override color over the theme default", () => {
    const html = renderPicker({ selectedTheme: "literary", selectedThemeColor: "#123456" });
    expect(html).toContain("#123456");
    expect(html).toContain("恢复默认");
  });

  it("renders gallery mode switch only for non-auto themes", () => {
    expect(renderPicker({ selectedTheme: "auto" })).not.toContain("配图排列");
    expect(renderPicker({ selectedTheme: "literary" })).toContain("配图排列");
  });
});
