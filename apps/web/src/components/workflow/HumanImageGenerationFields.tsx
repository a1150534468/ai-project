        ...
        <div className="grid grid-cols-4 overflow-hidden rounded-lg border border-[#d2d2d7]">
          {[1, 2, 3, 4].map((value) => (
            <button
              ...
              className={`h-9 border-r border-[#e5e7eb] text-sm font-semibold last:border-r-0 disabled:opacity-50 ${props.count === value ? "bg-brand-soft text-brand-ink" : "bg-white text-[#6e6e73]"}`}
            >
              {value}
            </button>
          ))}
