    ...
    return (
      <section className="grid gap-2" aria-label="原文内容">
        <div className="flex items-center justify-between gap-3">
          <span className="text-[10px] text-[#8a8a8f]">{props.sourceText.trim().length} 字</span>
        ...
        <textarea
          ...
          className="min-h-[160px] w-full resize-y rounded-lg border border-[#d2d2d7] bg-white px-3.5 py-3 text-sm leading-6 text-[#1d1d1f] outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/10"
  ...
  return (
    <section className="flex h-full min-h-[520px] flex-col bg-[#f7f8fa]" aria-label="原文内容">
      <div className="flex h-12 flex-none items-center justify-between gap-3 border-b border-[#e5e7eb] bg-white px-4 lg:px-5">
        <span className="text-xs text-[#8a8a8f]">{props.sourceText.trim().length} 字</span>
        <div className="grid grid-cols-2 rounded-lg bg-[#ececf0] p-1">
          {SOURCE_OPTIONS.map((option) => (
            <button
              key={option.key}
      ...
      <div className="flex min-h-0 flex-1 p-4 lg:p-5">
        <textarea
          ...
          value={props.sourceText}
          onChange={(event) => props.onSourceTextChange(event.target.value)}
          className="h-full min-h-[420px] w-full resize-none overflow-y-auto rounded-lg border border-[#d2d2d7] bg-white px-5 py-4 text-sm leading-7 text-[#1d1d1f] outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/10"
          placeholder={props.sourceFormat === "markdown" ? "粘贴 Markdown 内容" : "粘贴文章正文"}
        />
      </div>
    </section>
