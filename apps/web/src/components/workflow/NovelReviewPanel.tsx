  ...
  return (
    <section data-testid="novel-review-panel" className="grid min-w-0 content-start gap-3 rounded-lg border border-[#e8e8ed] bg-white p-4">
      ...
      <label className="grid gap-2 text-xs font-semibold text-ink-secondary">
        人工备注
        <textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={5} className="w-full resize-y rounded-lg border border-[#d2d2d7] p-3 text-sm leading-6 outline-none focus:border-brand/60" />
      </label>

      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <button type="button" disabled={!chapter || isSaving} onClick={() => onSave({ status: "pending", reviewNotes: notes })} className="h-9 rounded-lg border border-[#d2d2d7] px-3 text-xs font-semibold text-ink-secondary disabled:opacity-50">待审</button>
