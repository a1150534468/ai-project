...
describe("resolveClientMenuItems", () => {
  ...
  it("生图模块的页内 tab 作为三级菜单默认开启，旧电商图二级菜单已下线", () => {
    ...
    expect(tabs.map((tab) => tab.key)).toEqual(["workflow.image.general", "workflow.image.ecom", "workflow.image.product-extraction", "workflow.image.portrait", "workflow.image.try-on"]);
