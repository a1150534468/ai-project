 ...
 * 「素材库不建新表」一节）：它是 ImageAsset / VideoAsset / AudioAsset / PortraitOutput /
 * DubProject / CodexPetRun 这几张权威表的聚合读模型。查不到就是没有，源删则自然消失，
 * 孤儿结构性地不可能存在 —— 这正是知识库那 335 行归档触发器付出代价才没换到的性质。
 */
 ...
 * 六路归并 + 新素材随时插到最前面，offset 会漏行也会重行。
 */
