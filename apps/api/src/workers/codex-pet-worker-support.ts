 ...
 *
 * 这两个函数拆分前是 codex-pet-worker.ts 里的私有函数,而 recovery / billing / 主文件
 * 三边都要用,所以必须落在一个谁都能 import 的叶子上。它只从 codex-pet 门面取
 * `sanitizeCodexPetDiagnosticText` 一个纯函数,自己不碰 prisma / redis / 队列。
 *
 * 依赖方向:本文件(叶子)→ metrics → recovery / billing → codex-pet-worker.ts。
 */
