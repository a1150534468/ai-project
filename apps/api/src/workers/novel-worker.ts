...
export async function startNovelWorker(
  options: { readonly healthPort?: number | false } = {},
  ...
  const worker = createNovelWorker(async (job) => {
    if (job.data.type === "generation-task") {
      ...
      const step = await prisma.novelRunStep.findUnique({
        where: { id: stepId },
  ...
  const dispatchTimer = setInterval(() => {
    void dispatchNovelOutboxBatch(prisma)
      ...
      .catch((error) =>
        console.error(`[novel-worker] outbox error: ${error instanceof Error ? error.message : String(error)}`),
      );
  }, 1000);
  const recoveryTimer = setInterval(() => {
    void Promise.all([recoverInterruptedNovelSteps(prisma), recoverInterruptedNovelTasks(prisma)])
      .then(([stepCount, taskCount]) => {
        if (stepCount > 0 || taskCount > 0)
          console.warn(`[novel-worker] recovered ${stepCount} interrupted step(s), ${taskCount} generation task(s)`);
      })
      .catch((error) =>
        console.error(`[novel-worker] recovery error: ${error instanceof Error ? error.message : String(error)}`),
      );
  }, 15_000);
  lastDispatchAt = Date.now();
  ready = true;
  ...
  const healthPort =
    options.healthPort === false
      ? null
      : (options.healthPort ?? Number(process.env.NOVEL_WORKER_HEALTH_PORT ?? "8091"));
  const healthServer =
    healthPort == null
      ? null
