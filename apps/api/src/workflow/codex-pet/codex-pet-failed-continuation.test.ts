...
describe.skipIf(!databaseEnabled)("Codex pet failed continuation", () => {
  it("reuses durable base checkpoints and unlocks one prompt-upgrade replay without new calls", async () => {
...
describe.skipIf(!databaseEnabled)("Codex pet gate-failure continuation", () => {
  it("resets only the action groups the gate blamed and re-queues the derived jobs", async () => {
    ...
    // Blamed rows: cleared and rewound to attempt 0 so the runner regenerates them.
    expect(jobByKey.get("row-idle")).toMatchObject({ status: "failed", outputArtifactIds: [], attempt: 0, maxAttempts: 2 });
    expect(jobByKey.get("look-a")).toMatchObject({ status: "failed", outputArtifactIds: [], attempt: 0 });
    // An action the gate did not blame keeps the frames it already produced.
    ...
    // No image was regenerated by the reset itself, so the call count stands.
