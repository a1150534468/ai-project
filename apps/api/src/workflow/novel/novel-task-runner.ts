 ...
 * 对外导出的 9 个名字与拆分前逐字一致,`novel-task-runner.test.ts` / `novel-routes.ts` /
 * `novel/index.ts` 零改动。
 *
 * 分工:
 *  - novel-task-shared.ts   计费窄接口、任务行形状、四个共用叶子工具
 *  - novel-task-read.ts     对外 JSON 形状、项目详情聚合读、下一章序号
 *  - novel-task-context.ts  结构化资料读齐 + 压成上下文文本(含向量记忆降级)
 *  - novel-task-persist.ts  预留建单 与 结果写回,同一笔账的两端
...
export type { BillingForNovels, NovelTaskRow } from "./novel-task-shared.js";
export { estimateReserveChars } from "./novel-task-shared.js";
