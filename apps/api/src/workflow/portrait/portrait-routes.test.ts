...
describe("portrait workflow routes", () => {
  ...
  it("returns selectable models, per-model pricing, and prompt-free presets from options", async () => {
    ...
    expect(data.pricingByModel["doubao-seedream-5-0-260128"]).toEqual({ "2K": 0, "4K": 0 });
    // 模型不支持的档位不下发价格，前端无从展示无法下单的档位
    expect(data.pricingByModel["gpt-image-2"]).toEqual({ "1K": 0, "2K": 0 });
    expect(data.pricing["2K"]).toMatchObject({ resourceKey: "image_generation_2k", rate: 0 });
  ...
  it("resolves per-model admin pricing rows for options", async () => {
    ...
    expect(data.pricingByModel["gpt-image-2"]).toEqual({ "1K": 0, "2K": 33 });
    await app.close();
  });
