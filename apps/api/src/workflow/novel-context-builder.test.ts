...
describe("novel context builder", () => {
  it("builds fqxs-style layered context for a target chapter", () => {
    const payload = buildNovelGenerationContext({
      ...
      styleProfileText: "短句推进，结尾留钩子。",
      structuredContext: {
        contract: ["故事前提：沈氏后人追查家族旧案。"],
        world: ["寒泉院只能由沈氏血脉进入。"],
        characters: ["林岚：主角。", "赵虎：反派。"],
      },
      previousChapters: [
        {
          id: "c1",
