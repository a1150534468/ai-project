...
export function ClientMenusPage() {
  ...
  const workflowRows = rows.filter((row) => row.group === "workflow");
  ...
  const status = (row: api.ClientMenuItem, child = false) => {
    if (child && workflowParent && !workflowParent.visible) {
